...
class Article(db.Model):
    """
    Article model for database table articles
    """
    ...
    # definer = db.relationship('User', backref=db.backref('defined_article', lazy='dynamic'), foreign_keys=[definer_id])
    ...
    # revise = db.relationship('User', backref=db.backref('revised_article', lazy='dynamic'), foreign_keys=[reviser_id])
    ...
    def __init__(self, title, short_description, defined_user_id=0, tag=[], visible='Private', category="", author="",
                 source="",  description=[]):
        self.sg_tags = tag
        self.title = title
        self.author = author
